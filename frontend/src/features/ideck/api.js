...
/**
 * Fetch the panel status. Resolves even with the panel closed — check `state`
 * rather than catching. `access_denied` means the backend needs elevation.
 *
...
/**
 * List every key on the deck, in layout order. `client_x`/`client_y` are
 * populated only while the panel is open and restored.
 *
 * @param {{signal?: AbortSignal}} [options]
...
/**
 * Press one key by its layout name (case-insensitive). Unless verification is
 * off, resolves only once the panel's own log shows the press landing.
 *
...
/**
 * Check the panel accepts posted input, without pressing anything — posts a
 * mouse move only, so it's free of game side effects.
 *
