  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Read the screen as it is now, name every symbol with the trained
          classifier, OCR the figure on each scatter with PaddleOCR, and check it
          against the value range the loaded maths declares
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        {busy ? (
          <p className="text-muted-foreground text-xs">
            Reading the grid, then each scatter with PaddleOCR, then checking every
            figure against the loaded paytable. This takes up to a minute or so —
            longer on the first run of the backend, which also loads the OCR
            models.
          </p>
        ) : null}
