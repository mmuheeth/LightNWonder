...
# --- reading a clip in windows -------------------------------------------
#
# A whole-clip read could not be made to fit inside one request: a 90s clip
# drawing two caption bands is ~160 recognitions at ~2.8s, measured at 492s
# against a client that gives up at 290s. So a request reads at most
# CYCLIC_MESSAGES_TEXT_WINDOW_SECONDS of footage and says which stretch that
# was, and the caller pages. See that setting for what was tried instead.


def test_a_window_is_clamped_to_the_budget_and_says_so() -> None:
    """The caller may ask for the whole clip; the answer says what it got.

    That is what keeps paging free of any knowledge of the budget -- a client
    that had to know the window size would go wrong the moment the setting
    moved, and the failure would be a silently short reading rather than an
    error.
    """
    start, stop = text_service._window(0.0, 1000.0, 90.0)
    assert start == 0.0
    assert stop == settings.CYCLIC_MESSAGES_TEXT_WINDOW_SECONDS

    # Asking for nothing in particular reads the first window, so a caller
    # that never pages still gets a valid reading of the start of the clip.
    assert text_service._window(None, None, 90.0) == (
        0.0,
        settings.CYCLIC_MESSAGES_TEXT_WINDOW_SECONDS,
    )


def test_a_window_stops_at_the_end_of_the_clip() -> None:
    """Which is how a caller knows to stop: `to_seconds` reaching
    `duration_seconds` is the ordinary end of paging."""
    window = settings.CYCLIC_MESSAGES_TEXT_WINDOW_SECONDS
    assert text_service._window(window, None, window + 5.0) == (window, window + 5.0)


def test_a_window_past_the_end_is_empty_rather_than_backwards() -> None:
    """A start beyond the clip asks for nothing, which a caller paging
    correctly cannot do but a hand-typed offset can. Reported as an empty
    window rather than a negative one -- `to == from` is what a caller reads
    as 'no progress' and stops on, and a stop before the start would make
    `frames` nonsense."""
    start, stop = text_service._window(500.0, None, 90.0)
    assert start == 500.0
    assert stop == 500.0


def test_a_caption_keeps_the_key_it_was_grouped_under() -> None:
    """The key travels so a client stitching two windows can join on it.

    Not decoration: the text shown is the best-scoring *variant* of the
    readings behind a message, so two showings of one caption can differ
    character for character. A client joining on the text would leave the
    seam between two windows unjoined exactly when the recogniser wavered,
    and report one showing as two.
    """
    messages = text_service._group(
        [
            text_service._Read(0, 0.0, "Line 1 Pays 15", 90.0),
            text_service._Read(1, 1.0, "LINE 1 PAYS 15", 95.0),
        ]
    )

    assert len(messages) == 1
    assert messages[0].key == text_service.caption_key(messages[0].text)
    # And the distinct list counts by that same key rather than re-deriving
    # one from the variant it happens to show.
    captions = text_service._distinct(messages)
    assert len(captions) == 1
    assert captions[0].key == messages[0].key
