...
FULL = {
    "name": "HuffNPuffLink",
    "log": r"C:\logs\Game\HuffNPuffLink.log",
    "roi": {"cash_meter": [0.13, 0.75, 0.86, 0.78]},
    "button_targets": {"take_win": [0.124, 0.917]},
    "ideck": {"panel": "virtual_oled", "aliases": {"Spin": "Rebet"}},
}


def write(tmp_path: Path, document: Any, name: str = "HuffNPuffLink.json") -> Path:
    path = tmp_path / name
    path.write_text(
...
def test_every_block_is_read(tmp_path: Path) -> None:
    ...
    assert game.name == "HuffNPuffLink"
    assert game.ideck_panel == "virtual_oled"
    assert game.log_path == Path(r"C:\logs\Game\HuffNPuffLink.log")
...
def test_alias_keys_are_casefolded_so_lookups_need_not_be(tmp_path: Path) -> None:
    ...
    assert game.ideck_aliases["spin"] == "Rebet"
    # The target keeps the layout's own casing, which is what gets looked up.
    assert "Spin" not in game.ideck_aliases
...
def test_the_name_falls_back_to_the_filename(tmp_path: Path) -> None:
    game = load_game_config(write(tmp_path, {}, name="FortuneOx.json"))

    assert game.name == "FortuneOx"
...
def test_the_error_names_the_file_that_is_wrong(tmp_path: Path) -> None:
    ...
    with pytest.raises(GameConfigError, match=r"HuffNPuffLink\.json"):
        load_game_config(path)
